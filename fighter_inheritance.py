...
class Wizard(Fighter): #class that inherits from Fighter class
    def __init__(self, name, starting_health, weapon, shield, magic):
        super().__init__(name, starting_health, weapon, shield) #initialises the parent class
        self.magic = magic #additional attribute for magic power

    def random_attack(self):
        attack_power = random.randint(self.weapon/2, self.weapon*2)
        print('Attack power:', attack_power)
        return attack_power + self.magic #adds magic power to the attack power

you = Fighter('You',100,60,20)
wiz = Wizard('Troll',100,30,10,50)

you.report()
wiz.report()

while True: #a loop that runs until the health of the wizard is less than or equal to 0
    print("YOU ATTACK THE", wiz.name + "!")
    wiz.defend(you.skill_attack()) #sets the health of the wizard to the current health minus the attack power of the skill attack
    wiz.report() #prints the health of the troll
    time.sleep(2) #pauses the program for 2 second
    print('----------------------------------')
    if wiz.is_dead(): #checks if the health of the wizard is less than or equal to 0
        print("YOU WIN!")
        break #breaks the loop if the health of the troll is less than or equal to 0
    print(wiz.name + " ATTACKS YOU!")
    you.defend(wiz.skill_attack()) #sets your health to the current health minus the attack power of the skill attack
